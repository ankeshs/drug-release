...
%   Detailed explanation goes here
    ...
    if( ismember('fluid partition',params) ) 
      Kf=list(li);   
      li=li+1;
    end
    ...
    M=release(Co, Avd, Bvd, Cvd, Dvd, D01, Kr, Kf, dens, hp, ht, tf, dt);
    ...
    if(Ttw<=0)
      ...
      if(Ttw>0)
	if(tdi > 1)
	    M.result.numerical('int1').set('interp', {strcat('range(',num2str(t(tdi-1)),', 1 ,',num2str(t(tdi)),')')});
	    n1=M.result.numerical('int1').getReal;
	    minI=1;
	    i=1;
	    while(i<=length(n1))
	...
	if(tdd < length(n))
	    M.result.numerical('int1').set('interp', {strcat('range(',num2str(t(tdd)),', 1 ,',num2str(t(tdd+1)),')')});
	    n1=M.result.numerical('int1').getReal;
	    minI=1;
	    i=1;
	    while(i<=length(n1))
